...
try:
    ...
    print(
        "Unable to load cologne_phonetics. German language phonetic matching won't be available! Install with pip install cologne_phonetics if needed.",
        e,
    )
...
class SemScore:
    ...
    def score_nli(self, refs, hyps, direction=None, formula="e"):
        ...
        with torch.no_grad():
            ...
            if direction == "rh":
                ...
                final_score = [
                    (s1 + s2) / 2.0 for s1, s2 in zip(probs_rh["e"], probs_hr["e"])
                ]
    ...
    def score_all(self, refs, hyps, lang="en"):
        bert_scores = calculate_bert_score(
            refs, hyps, self.device, lang=lang
        ).tolist()  # Bert Score
        bert_scores = self.min_max_normalize(bert_scores, [-0.1180, 1])

        phonetic_scores = []
        for ref, hyp in zip(refs, hyps):
            phonetic_score = calculate_phonetic_similarity(ref, hyp, lang=lang)
            phonetic_scores.append(phonetic_score)
        phonetic_scores = self.min_max_normalize(
            phonetic_scores, [0.5, 1]
        )  # phonetic score

        nli_scores = self.score_nli(refs, hyps, formula="e")
        nli_scores = self.min_max_normalize(
            nli_scores, [0.0028752246871590614, 0.9661698341369629]
        )

        combined_scores = [
            self.nli_weight * nli
            + self.bert_weight * bert
            + self.phonetic_weight * phon
    ...
    def min_max_normalize(self, scores, thre):
        assert len(scores) != 0
        normalized_scores = [
            max(min((score - thre[0]) / (thre[1] - thre[0]), 1), 0) for score in scores
        ]

        return normalized_scores


def calculate_bert_score(reference, hypothesis, device, lang="en"):
    P, R, F1 = bert_score(
        reference, hypothesis, lang=lang, rescale_with_baseline=True, device=device
    )
    return F1
...
def calculate_phonetic_similarity(reference, hypothesis, lang="en"):
    if lang == "de":
        hypo_soundex = "".join(
            [i[1] for i in cologne_phonetics.encode(hypothesis, concat=True)]
        )
        ref_soundex = "".join(
            [i[1] for i in cologne_phonetics.encode(reference, concat=True)]
        )
    else:
