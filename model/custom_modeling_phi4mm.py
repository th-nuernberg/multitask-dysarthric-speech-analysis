...
from model.modeling_outputs import CustomCausalLMOutputWithPast
    ...
    def get_img_features(
        ...
        if TYPE_FEATURE == "cls_patch":
            if self.image_token_compression is not None:
                ...
                patch_feature = self.image_token_compression(patch_feature)
                patch_feature = patch_feature.view(
                    -1, patch_feature.size(-2) * patch_feature.size(-1)
                )
        ...
        **kwargs,
        ...
        if select:
            if hd_transform:
                ...
                assert all(
                    [
                        _img_set_tensor.shape[0] == 1
                        for _img_set_tensor in img_set_tensor
                    ]
        ...
        if fake_image_forward and self.training:
            hidden_states = (
                ...
                + (
                    0
                    * img_set_tensor[0].to(hidden_states.dtype).to(hidden_states.device)
                ).sum()
            )
...
class Phi4MMAudioEmbedding(nn.Module):
    ...
    def get_audio_features(
        ...
        if isinstance(self.audio_projection, nn.Sequential):
            ...
            audio_set_tensor = self.audio_projection[audio_projection_mode](
                audio_features
            )
        else:
            raise NotImplementedError
    ...
    def forward(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            positions = torch.nonzero(
                input_ids == _AUDIO_SPECIAL_TOKEN_ID, as_tuple=False
            )
            positions_tuple = torch.nonzero(
        ...
        if len(positions.tolist()) > 0:
            ...
            if self.training:
                audio_embeds = (
                    torch.zeros(1, 500, self.audio_dim_in)
                    .to(target_device)
                    .to(target_dtype)
                )
                audio_attention_mask = audio_embeds.new_ones(
                    audio_embeds.size()[:2]
                ).long()
                audio_set_tensor = self.get_audio_features(
        ...
        if self.drop is not None:
            hidden_states = self.drop(hidden_states)
        # logger.error(f"Audio encoder hidden states: {hidden_states.shape=}")
        return hidden_states
    ...
    def forward(
        ...
        if input_image_embeds is not None and input_audio_embeds is not None:
            ...
            hidden_states = image_hidden_states * image_position_mask.to(
                dtype
            ).unsqueeze(-1) + audio_hidden_states * non_image_position_mask.to(
                dtype
            ).unsqueeze(-1)
    ...
    def _init_rope(self):
        if self.rope_scaling is None:
            ...
            if scaling_type == "longrope":
                self.rotary_emb = Phi4MMLongRoPEScaledRotaryEmbedding(
                    self.rotary_ndims, self.config
                )
            else:
...
class Phi4MMDecoderLayer(nn.Module):
    def __init__(self, config: Phi4MMConfig, layer_idx: int):
        ...
        self.config = config
        self.self_attn = PHI4MM_ATTENTION_CLASSES[config._attn_implementation](
...
class Phi4MMModel(Phi4MMPreTrainedModel):
    ...
    def __init__(self, config: Phi4MMConfig):
        ...
        # TODO: Maybe we should put loras on the audio embedding?
        ...
        self.layers = nn.ModuleList(
            [
                Phi4MMDecoderLayer(config, layer_idx)
...
class Phi4MMForCausalLM(Phi4MMPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.latest_cos_sim = -99
        self.latest_kld_consistency_loss = -99
        self.latest_aux_clf_loss = -99
        self.latest_aux_reg_loss = -99
        ...
        # Custom LoRAs for audio encoder
        if getattr(config, "audio_encoder_lora", None) is not None:
            logger.info(
                "Audio encoder LoRA config detected. Adding adapters to audio encoder."
            )
            audio_encoder_lora_config = LoraConfig(
                r=config.audio_encoder_lora["r"],
                lora_alpha=config.audio_encoder_lora["lora_alpha"],
                target_modules=config.audio_encoder_lora["layer"],
                lora_dropout=config.audio_encoder_lora["dp"],
                task_type="CAUSAL_LM",
            )
            peft_model.base_model.active_adapter.append("audio")
            peft_model.add_adapter("audio", audio_encoder_lora_config)
            self.config.audio_encoder_lora["r"] = config.audio_encoder_lora["r"]
            self.config.audio_encoder_lora["lora_alpha"] = config.audio_encoder_lora[
                "lora_alpha"
            ]
            self.config.audio_encoder_lora["layer"] = config.audio_encoder_lora["layer"]
            self.config.audio_encoder_lora["dp"] = config.audio_encoder_lora["dp"]
            peft_model.print_trainable_parameters()

        # Classification heads
        self.clf_loss_weight = (
            config.clf_loss_weight
        )  # Overall weight of LLM loss vs. classifier loss
        self.clf_task_weights = config.clf_task_weights  # Weighting of individual classification tasks Dict: {"taskname": "weight for loss"}
        clf_tasks = config.clf_tasks  # Dict: {"taskname": num_classes}
        if clf_tasks is not None:
            self.clf_loss_fn = nn.CrossEntropyLoss()
            proj_dim = 1024
            self.down_proj = nn.Linear(config.hidden_size, proj_dim)
            self.classification_heads = nn.ModuleDict(
                {
                    task: nn.Linear(proj_dim, num_classes)
                    for task, num_classes in clf_tasks.items()
                }
            )
            logger.info(
                f"Created auxiliary classification heads:\n{self.classification_heads}"
            )
            logger.info(f"{config.output_hidden_states=}, {config.clf_use_enc_embeds=}")

        # Auxiliary regression task
        if (
            hasattr(config, "use_aux_regression_task")
            and config.use_aux_regression_task
        ):
            self.aux_reg = nn.Sequential(
                nn.Linear(config.aux_reg_hidden_size, 256),
                nn.ReLU(),
                nn.Linear(256, config.aux_reg_num_pred),
            )
            self.aux_reg_loss_fn = nn.MSELoss()
            self.aux_reg_loss_weight = config.aux_reg_loss_weight
            logger.info(f"Created auxiliary regression model:\n{self.aux_reg}")
    ...
    def forward(
        ...
        image_attention_mask=None,
        input_audio_embeds: Optional[torch.FloatTensor] = None,
        input_audio_embeds_other: Optional[torch.FloatTensor] = None,
        audio_embed_sizes=None,
        audio_attention_mask=None,
        input_mode=None,
        ...
        aux_reg_labels: Optional[torch.LongTensor] = None,
        aux_reg_mask: Optional[torch.BoolTensor] = None,
        aux_reg_embeds: Optional[torch.FloatTensor] = None,
        ...
        # Apply aux regression model
        aux_reg_loss = None
        if (
            hasattr(self.config, "use_aux_regression_task")
            and self.config.use_aux_regression_task
        ):
            if aux_reg_mask is not None and aux_reg_mask.any():
                aux_reg_preds = self.aux_reg(aux_reg_embeds)
                aux_reg_loss = self.aux_reg_loss_fn(aux_reg_preds, aux_reg_labels)
                aux_reg_loss = aux_reg_loss * aux_reg_mask
                aux_reg_loss = aux_reg_loss.sum() / aux_reg_mask.sum().clamp(min=1)
                self.latest_aux_reg_loss = aux_reg_loss

        # Apply classification head
        total_cls_loss = None
        clf_logits = None
        if self.config.clf_tasks is not None:
            if self.config.clf_use_enc_embeds:
                audio_embeds = outputs.hidden_states[0]  # (B, T, C)
                clf_logits = self._get_aux_clf_logits_by_task(audio_embeds)
            else:
                clf_logits = self._get_aux_clf_logits_by_task(hidden_states)

            clf_losses = {}
            # logger.error(f"{cls_labels=}")
            if cls_labels is not None and tasks is not None:
                # This masks the pure "asr" task completely and assembles task-specific masks for all others,
                # so that only the elements from the task corresponding to a specific classification head
                # are considered in the loss computation
                # By replacing "asr_", the joint ASR + classification tasks get mapped to the corresponding classification head
                task_masks = {
                    k: torch.zeros(
                        len(tasks), dtype=torch.bool, device=input_ids.device
                    )
                    if k == "asr"
                    else torch.tensor(
                        [key.replace("asr_", "") == k for key in tasks],
                        dtype=torch.bool,
                        device=input_ids.device,
                    )
                    for k in set(tasks)
                }
                for task, logits in clf_logits.items():
                    task_mask = task_masks.get(task, None)
                    if task_mask is not None:
                        valid_mask = (
                            (cls_labels >= 0)
                            & (cls_labels < logits.size(1))
                            & task_mask
                        )
                        if valid_mask.any():
                            logit_view = logits[valid_mask].view(-1, logits.size(-1))
                            lbl_view = cls_labels[valid_mask].view(-1)
                            l = self.clf_loss_fn(logit_view, lbl_view)
                            clf_losses[task] = l
                total_cls_loss = sum(
                    self.clf_task_weights[task] * loss
                    for task, loss in clf_losses.items()
                )
                if not self._is_valid_loss(total_cls_loss):
                    total_cls_loss = None
                # logger.error(f"{total_cls_loss=} {type(total_cls_loss)=}")
        ...
        if labels is not None:
            ...
            if aux_reg_loss is not None:
                loss = loss + self.aux_reg_loss_weight * aux_reg_loss

            if total_cls_loss is not None:
                if self.config.use_gradient_consistency_loss:
                    # Apply gradient consistency loss here
                    loss = self._gradient_consistency_loss(
                        loss, total_cls_loss, exclude_input_embeds=True
                    )
                else:
                    loss = loss + self.clf_loss_weight * total_cls_loss
                    self.latest_aux_clf_loss = total_cls_loss

            if (
                self.config.use_kld_consistency_loss
                and input_audio_embeds_other is not None
            ):
                # Apply Specaugment consistency loss here
                outputs_other = self.model(
                    input_ids=input_ids,
                    attention_mask=attention_mask,
                    position_ids=position_ids,
                    past_key_values=past_key_values,
                    ...
                    image_sizes=image_sizes,
                    image_attention_mask=image_attention_mask,
                    input_audio_embeds=input_audio_embeds_other,
                    audio_embed_sizes=audio_embed_sizes,
                    audio_attention_mask=audio_attention_mask,
                    audio_projection_mode=audio_projection_mode,
                    use_cache=use_cache,
                ...
                hidden_states_other = outputs_other[0]
                logits_other = self.lm_head(
                    hidden_states_other[:, -num_logits_to_keep:, :]
                )
                loss_other = self.loss_function(logits_other, labels, self.vocab_size)
                kld_consistency_loss = self._consistency_kld(
                    hidden_states, hidden_states_other
                )

                # This is equivalent to Eq. 3 in CR-CTC: Consistency regularization on CTC for improved speech recognition
                # https://arxiv.org/abs/2410.05101
                loss = (
                    0.5 * (loss + loss_other)
                    + self.config.kld_consistency_weight * kld_consistency_loss
                )
    ...
    def _is_valid_loss(loss):
        return isinstance(loss, torch.Tensor) and loss.requires_grad and loss.dim() == 0

    def _compute_gradient(
        self, loss, exclude_input_embeds=False, restrict_to_shared=False
    ):
        if restrict_to_shared:
            params = [
                p
                for name, p in self.named_parameters()
                if p.requires_grad and "classification_heads" in name
            ]
        elif exclude_input_embeds:
            params = [
                p
                for name, p in self.named_parameters()
                if p.requires_grad and "embed_tokens" not in name
            ]
        else:
            params = [p for p in self.parameters() if p.requires_grad]

        grads = torch.autograd.grad(
            loss, params, retain_graph=True, create_graph=True, allow_unused=True
        )
    ...
    def _gradient_consistency_loss(
        self, loss1, loss2, exclude_input_embeds=False, restrict_to_shared=False
    ):
        g1 = self._compute_gradient(loss1, exclude_input_embeds, restrict_to_shared)
        g2 = self._compute_gradient(loss2, exclude_input_embeds, restrict_to_shared)
        cos_sim = F.cosine_similarity(g1, g2, dim=0)
        logger.error(f"{g1.shape=}, {g2.shape=}, {cos_sim=}")

        # Save for logging
        self.latest_cos_sim = cos_sim.detach()
        return loss1 + loss2 - self.clf_loss_weight * cos_sim

    def _consistency_kld(self, z1, z2):
        """
        Uses bidirectional KL Divergence, the symmetric version of the Kullback-Leibler (KL) divergence
        :param z1: encoder(spec_1)
        :param z2: encoder(spec_2)
        :return: bidirectional KLD loss
        """
        z1 = F.log_softmax(z1, dim=-1)
        z2 = F.log_softmax(z2, dim=-1)
        loss = 0.5 * (
            F.kl_div(z1, z2.exp(), reduction="batchmean")
            + F.kl_div(z2, z1.exp(), reduction="batchmean")
        )
        # logger.error(f"{z1.shape=}, {z2.shape=}, {loss=}")
        self.latest_kld_consistency_loss = loss.detach()
        # if random.random() < 0.01:  # ~1% of the time
        #     logger.info(f"KLD consistency loss = {loss}")
        #     wandb.log({"kld_consistency_loss": loss.item()}, commit=False)
        return loss

    def _get_aux_clf_logits_by_task(self, hidden_states):
        proj_hidden_states = self.down_proj(hidden_states)
        pooled_output = proj_hidden_states.mean(dim=1)
        clf_logits = {
            task: head(pooled_output)
...
if __name__ == "__main__":
    ...
    config.clf_loss_weight = 1.0  # Overall weight of LLM loss vs. classifier loss
    config.clf_task_weights = {
        "intelligibility": 1.0,
        "naturalness": 1.0,
    }  # Weighting of individual classification tasks Dict: {"taskname": "weight for loss"}
    config.clf_tasks = {
        "intelligibility": 6,
        "naturalness": 7,
    }  # Dict: {"taskname": num_classes}
    config.clf_token_id = 200029  # Token Id of <|clf|>
    print(config)
    ...
    print(model.classification_heads)
