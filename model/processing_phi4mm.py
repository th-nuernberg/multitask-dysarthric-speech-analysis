...
class Phi4MMImageProcessor(BaseImageProcessor):
    ...
    image_processor_type = "phi4mm_image"
...
class Phi4MMSpecAugmentAudioFeatureExtractor(SequenceFeatureExtractor):
    model_input_names = [
        "input_audio_embeds",
        "input_audio_embeds_other",
        "audio_embed_sizes",
        "audio_attention_mask",
    ]
    feature_extractor_type = "phi4mm_specaug"
    ...
    def __init__(
        ...
        # Values corresponding to Lhotse:
        # https://github.com/lhotse-speech/lhotse/blob/master/lhotse/dataset/signal_transforms.py
        # In the CR-CTC Paper, 2.5x the lhotse config  is applied for time masking
        # self.time_warping = T.TimeStretch()
        self.cr_ctc_adjustment = kwargs.get("cr_ctc_adjustment", 2.5)
        self.num_time_masking = int(10 * self.cr_ctc_adjustment)
        self.num_freq_masking = 2
        self.time_warp_factor = 80
        self.specaug_p = 0.9
    ...
    def __call__(
        ...
        time_masking = T.TimeMasking(
            time_mask_param=100, p=self.cr_ctc_adjustment * 0.15
        )
        freq_masking = T.FrequencyMasking(freq_mask_param=27)

        for audio_data, sample_rate in audios:
            # Randomly choose whether Specaugment is applied
            if random.random() > self.specaug_p:
                specaugment_mode = "clean_only"
            audio_embeds, audio_embeds2 = self._extract_specaugment_features(
                audio_data,
                sample_rate,
                time_masking,
                freq_masking,
                mode=specaugment_mode,
            )
            returned_input_audio_embeds2.append(torch.tensor(audio_embeds2))
        ...
        returned_input_audio_embeds2 = pad_sequence(
            returned_input_audio_embeds2, batch_first=True
        )
        ...
        data = {
            ...
            "input_audio_embeds_other": returned_input_audio_embeds2,
        }
    ...
    def _extract_specaugment_features(
        self, wav, fs, time_masking, freq_masking, mode="clean_vs_aug"
    ):
        """Extract augmented log filterbank features from waveform.
        Args:
            wav (1D array): waveform of the input
            fs (int): sampling rate of the waveform, 16000 or 8000.
                If fs=8000, the waveform will be resampled to 16000Hz.
            mode (str): Supports 3 modes:
        ...
        Output:
            log_fbank (3D array): a TxD matrix of augmented log Mel filterbank features.
                D=80, and T is the number of frames.
        """
        # 1. Clean vs. augmented spectrogram
        if mode == "clean_vs_aug":
            spec = self._extract_spectrogram(wav, fs)
            spec_power = spec**2
            ...
            aug_spec = (
                self._apply_specaugment(
                    torch.from_numpy(spec), time_masking, freq_masking
                )
                .cpu()
                .detach()
                .numpy()
            )
            aug_spec_power = aug_spec**2

            aug_fbank_power = np.clip(aug_spec_power.dot(self._mel), 1.0, None)
            aug_log_fbank = np.log(aug_fbank_power).astype(np.float32)
            return log_fbank, aug_log_fbank
        # 2. Augmented vs. augmented spectrogram
        elif mode == "aug_vs_aug":
            spec = self._extract_spectrogram(wav, fs)
            aug_spec1, aug_spec2 = self._apply_specaugment_twice(
                torch.from_numpy(spec), time_masking, freq_masking
            )
            aug_spec1_power = aug_spec1.cpu().detach().numpy() ** 2
            aug_spec2_power = aug_spec2.cpu().detach().numpy() ** 2

            aug_fbank_power1 = np.clip(aug_spec1_power.dot(self._mel), 1.0, None)
            aug_log_fbank1 = np.log(aug_fbank_power1).astype(np.float32)

            aug_fbank_power2 = np.clip(aug_spec2_power.dot(self._mel), 1.0, None)
            aug_log_fbank2 = np.log(aug_fbank_power2).astype(np.float32)
            return aug_log_fbank1, aug_log_fbank2
        # 3. Only return a single augmented spectrogram
        elif mode == "aug_only":
            spec = self._extract_spectrogram(wav, fs)
            aug_spec = (
                self._apply_specaugment(
            ...
            aug_fbank_power = np.clip(aug_spec_power.dot(self._mel), 1.0, None)
            aug_log_fbank = np.log(aug_fbank_power).astype(np.float32)
            return aug_log_fbank, aug_log_fbank
        elif mode == "clean_only":
            log_fbank = self._extract_features(wav, fs)
            return log_fbank, log_fbank
        else:
    ...
    def _apply_specaugment(self, spec, time_masking, freq_masking):
        spec = self._time_warp(spec).transpose(0, 1)
        for _ in range(self.num_time_masking):
            spec = time_masking(spec)
        for _ in range(self.num_freq_masking):
            spec = freq_masking(spec)
        return spec.transpose(0, 1)

    def _apply_specaugment_twice(self, spec, time_masking, freq_masking):
        """
        Apply time and frequency masking twice but
        Intended for consistency comparison similar to
        "CR-CTC: Consistency regularization on CTC for improved speech recognition" https://arxiv.org/abs/2410.05101
        Spec dim is (T, F)
        """
        warped_spec = self._time_warp(spec)
        spec1 = warped_spec.detach().clone().transpose(0, 1)
        spec2 = warped_spec.detach().clone().transpose(0, 1)
        # From here spec dims are (F, T)
        for _ in range(self.num_time_masking):
            spec1 = time_masking(spec1)
            spec2 = time_masking(spec2)

        for _ in range(self.num_freq_masking):
            spec1 = freq_masking(spec1)
            spec2 = freq_masking(spec2)
        # Transpose back to (T, F)
        return spec1.transpose(0, 1), spec2.transpose(0, 1)
    ...
    def _time_warp(self, features: torch.Tensor) -> torch.Tensor:
        ...
        taken from: https://github.com/lhotse-speech/lhotse/blob/master/lhotse/dataset/signal_transforms.py
...
class CustomPhi4MMProcessor(ProcessorMixin):
    r"""
    ...
    audio_processor_class = "AutoFeatureExtractor"  # Phi4MMSpecAugmentAudioFeatureExtractor will be registered later
    processor_class = "CustomPhi4MMProcessor"
    ...
    def __call__(
        ...
        specaugment_mode: Optional[str] = "clean_vs_aug",
    ) -> BatchFeature:
        """
        ...
        Returns:
            ...
            - **input_audio_embeds** -- Possibly augmented audio embeddings to be fed to a model.
            - **input_audio_embeds_other** -- Other possibly augmented audio embeddings to be fed to a model.
        ...
        audio_inputs = (
            self.audio_processor(
                audios, return_tensors=return_tensors, specaugment_mode=specaugment_mode
            )
    ...
    def _convert_images_audios_text_to_inputs(
        ...
        if len(audios) > 0:
            ...
            input_audio_embeds_other = audios["input_audio_embeds_other"]
            ...
            input_audio_embeds_other = torch.tensor([])
        ...
        data = {
            ...
            "input_audio_embeds_other": input_audio_embeds_other,
...
AutoImageProcessor.register("phi4mm_image", Phi4MMImageProcessor)
AutoFeatureExtractor.register("phi4mm_specaug", Phi4MMSpecAugmentAudioFeatureExtractor)
